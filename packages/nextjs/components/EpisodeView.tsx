...
/**
 * Resolves `/<slug>` against the contract's `slugToId` mapping (O(1) view),
 * reads the episode struct, then fetches the manifest JSON from IPFS to
 * populate description / participants / files / etc.
 *
 * The whole thing is client-side — that's what lets a single static-exported
 * HTML shell render *any* registered slug. The dedicated `app/[slug]` route
 * pre-renders one shell per known slug (for unfurl), but `app/not-found.tsx`
 * mounts this same component for slugs that didn't exist at export time, so
 * episodes registered after the last `yarn ipfs` still resolve on IPFS — the
 * lookup is identical, only the entry point differs. `notFound` is what we
 * render when the slug doesn't resolve: the route passes a component that
 * throws Next's `notFound()` (real 404 status on the dynamic deploy); the
 * static fallback passes inline 404 markup.
 *
 * Live: if `live == ep.id` we ignore the manifest and embed the live HLS
 * player. VOD: embed a plain `<video controls src=ipfsGatewayUrl>` so seeking,
 ...
 */
export const EpisodeView = ({ slug, notFound }: { slug: string; notFound: ReactNode }) => {
  ...
  // is `undefined` — the old check fired notFound and returned a 404 body
  // before the client ever hydrated. `isError` covers the other side:
  // `getEpisodeBySlug` reverts (doesn't return zero) when the slug isn't
  // registered, so we treat the revert as "not found" too.
  const {
    data: episode,
    isFetched,
    isError,
